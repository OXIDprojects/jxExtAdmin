...
[{ if $oConfig->getConfigParam("sJxExtAdminShowArticleVariant") }]
    ...
    <td class="[{ $listclass}]"><input type="text" class="editinput" size="20" maxlength="[{$listitem->oxarticles__oxmpn->fldmax_length}]" name="editval[[{ $listitem->oxarticles__oxid->value}]][oxarticles__oxmpn]" value="[{$listitem->oxarticles__oxmpn->value}]" [{ $readonly }]></td>
